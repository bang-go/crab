...
func WatchSignal(done chan struct{}, extBagger ...bag.Bagger) {
	...
	case <-done:
		break
	case s := <-sigChan:
		log.FrameLogger.Info("received signal", logx.String("sig", s.String()))
		break
	}
...
func gracefulShutdown(sig chan os.Signal, bagger ...bag.Bagger) {
	...
	select {
	case <-ch:
		log.FrameLogger.Info("graceful stop finish")
		break
	case <-time.After(MaxWaitTime):
		log.FrameLogger.Error("The maximum wait time exceeded")
		break
	}
