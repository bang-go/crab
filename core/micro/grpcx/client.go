...
func (c *ClientEntity) Dial() (conn *grpc.ClientConn, err error) {
	...
	if c.Trace {
		baseClientOption = append(baseClientOption, grpc.WithChainUnaryInterceptor(grpctrace.UnaryClientInterceptor()), grpc.WithChainStreamInterceptor(grpctrace.StreamClientInterceptor()))
	}
